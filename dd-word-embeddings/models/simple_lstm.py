...
class SimpleLSTM(nn.Module):
    def __init__(self, embedding_mat, hidden_size, out_size, batch_size, lstm_layers=1, use_cuda=False):
        super(SimpleLSTM, self).__init__()
        self.use_cuda = use_cuda

        torch_embedding_mat = torch.Tensor(embedding_mat)
        if use_cuda:
            torch_embedding_mat.cuda()
        emb_layer, num_embeddings, embedding_dim = pytorch_util.create_emb_layer(torch_embedding_mat, non_trainable=True)
    ...
    def forward(self, x):
        ...
        sorted_x, sorted_lengths, sorted_indices = pytorch_util.sort_by_sequence_length(x, self.batch_size, self.use_cuda)

        # encode
        embeds = self.word_embeddings(sorted_x)

        # pack
        packed = nn.utils.rnn.pack_padded_sequence(embeds, sorted_lengths, batch_first=True)
