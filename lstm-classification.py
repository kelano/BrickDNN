...
def train_epoch(key, train_X, train_Y, batch_size, current_learning_rate, batch_losses, optimizer, cost, sess, x, y,
                keep_prob, learning_rate, runtime_embedding, tf_embedding):
    ...
    for batch in batches:
        ...
        if "dnn" in key:
            _, batch_loss = sess.run([optimizer, cost], feed_dict={x: batch_x, y: batch_y, keep_prob: 0.5,
                                                                   ...
                                                                   runtime_embedding: tf_embedding})
        else:
...
def run_batch_eval(key, X, Y, batch_size, prediction, cost, sess, x, y, keep_prob, runtime_embedding, tf_embedding):
    batch_losses = []
    tot_cor = 0
    tot = 0
    ...
    for batch in batches:
        ...
        preds = sess.run(prediction, feed_dict={x: batch_x, y: batch_y, keep_prob: 1.0, runtime_embedding: tf_embedding})
        xval_loss = sess.run(cost, feed_dict={x: batch_x, y: batch_y, keep_prob: 1.0, runtime_embedding: tf_embedding})
...
def run_batch_xval(key, dev_X, dev_Y, batch_size, xval_losses, prediction, cost, sess, x, y, keep_prob, runtime_embedding, tf_embedding):
    batch_count = 0
    batches = get_batch_indices(batch_size, len(dev_X))
    ...
    for batch in batches:
        batch_x = dev_X[batch[0]:batch[1]]
        batch_y = dev_Y[batch[0]:batch[1]]
        ...
        batch_xval_losses.append(xval_loss)


    xval_loss = np.mean(batch_xval_losses)
    acc = float(tot_cor) / tot

    print('XVAL Acc: %f (%d/%d) Loss: %f' % (acc, tot_cor, tot, xval_loss))
...
def train_neural_network_new(key, model):
    ...
    y = tf.placeholder(tf.int32, [batch_size, numClasses])
    keep_prob = tf.placeholder(tf.float32)
    learning_rate = tf.placeholder(tf.float32, shape=[])
    ...
    runtime_embedding = tf.placeholder(tf.float32, shape=(6047, 300))

    # x_embedding = tf.Variable(tf.zeros([batch_size, max_seq_len, encoding_size]), dtype=tf.float32)
    x_embedding = tf.nn.embedding_lookup(runtime_embedding, x)

    lstmCell = tf.contrib.rnn.BasicLSTMCell(lstmUnits)
    lstmCell = tf.contrib.rnn.DropoutWrapper(cell=lstmCell, output_keep_prob=keep_prob)
    value, _ = tf.nn.dynamic_rnn(lstmCell, x_embedding, dtype=tf.float32)
    ...
    with tf.Session() as sess:
        ...
        while epoch < hm_epochs and not stop_training:
            ...
            num_batches = train_epoch(key, train_X, train_Y, batch_size, current_learning_rate, batch_losses, optimizer,
                                      cost, sess, x, y, keep_prob, learning_rate, runtime_embedding, tf_embedding)

            xval_loss, xval_acc, _ = run_batch_eval(key, dev_X, dev_Y, batch_size, prediction, cost, sess,
                                                              x, y, keep_prob, runtime_embedding, tf_embedding)

            xval_losses.append(xval_loss)
            if len(xval_losses) > 1 and xval_losses[-1] > xval_losses[-1]:
...
def test_neural_network_new(eval_map):
    for key, value in eval_map.items():
        ...
        with tf.Session() as sess:
            ...
            test_loss, test_acc, test_losses = run_batch_eval(key, test_X, test_Y, batch_size, prediction, cost, sess, x,
                                                 y, keep_prob, runtime_embedding, tf_embedding)
...
def build_dataset():
    ...
    import fasttext
    print('loading fasttest')
    word_2_vec, word_to_idx, idx_to_vec, _ = fasttext.load_vectors()
    print('done loading')
    ...
    save_dataset(train_X, train_Y, dev_X, dev_Y, test_X, test_Y, name='lstmdataset')
    # quit()
...
Y_col = list(range(20, 20 + len(theme_bow_vec)))
