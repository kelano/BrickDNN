...
def perform_analysis(X_cols, Y_col, np_dataset, word_bow_vec):
    Y_col = Y_col[0]
    pcc_dict = {}
    ...
    for X_col in X_cols:
        ...
        # plt.show()
        pcc_dict[word_bow_vec[X_col]] = abs(np.corrcoef(all_x, all_y)[0][1])

    print(pcc_dict)
    sorted_pcc_dict = sorted(pcc_dict.items(), key=operator.itemgetter(1))

    import json
    with open('file.txt', 'w') as file:
        file.write(json.dumps(sorted_pcc_dict))
